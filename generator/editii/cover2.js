...
const ED = { complet: null, C: ['Construcții', 'Construction'], H: ['HoReCa (hoteluri și restaurante)', 'Hotels and restaurants'], T: ['Transport, logistică și curierat', 'Transport, logistics and courier'], R: ['Comerț (retail)', 'Retail'], P: ['Producție (industrie)', 'Manufacturing'] };
const key = process.argv[2], out = process.argv[3], dom = ED[key];
...
add(new Paragraph({ spacing: { before: 0, after: 700 }, children: [] }));
add(cov('LIMBA ROMÂNĂ PENTRU STRĂINI', { bold: true, color: C.dk, size: 48 }, 120),
  cov('Romanian Language for Foreign Workers', { italics: true, color: '555555', size: 26 }, 400),
  ...
  cov('Complete course book — Modules 1–14 · Levels A1–A2 · 52 sessions · 156 hours', { italics: true, color: '555555', size: 22 }, 240));
if (dom) add(cov(`EDIȚIA PENTRU DOMENIUL: ${dom[0].toUpperCase()}`, { bold: true, color: C.dk, size: 28 }, 60), cov(`Edition for the field of work: ${dom[1]}`, { italics: true, color: '555555', size: 22 }, 360));
else add(cov('EDIȚIA COMPLETĂ — cu toate cele cinci variante ale Modulului 9', { bold: true, color: C.dk, size: 26 }, 360));
...
const m9 = dom ? `Domeniul de activitate: ${dom[0]}` : 'Domeniul de activitate — 5 variante: Construcții · HoReCa · Transport, logistică, curierat · Comerț (retail) · Producție, industrie';
